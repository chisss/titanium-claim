...
[系统全景](#titanium-是什么) · [服务索引](#微服务索引) · [架构边界](#架构与领域边界) · [当前服务](#titanium-claim) · [快速开始](#快速开始)
...
> **理赔域**：覆盖理赔报案、立案、调查、定损、责任审核、给付决定和结案的完整生命周期。

| 属性 | 内容 |
|---|---|
| 限界上下文 | 理赔域（核心域） |
| 核心模型 | ClaimCase、Compensation |
| 主要上游 | Policy、Customer、Document |
| 主要下游 | Payment、Regulatory、Reinsurance |
| 默认地址 | [`http://localhost:8083/claim`](http://localhost:8083/claim) |
| GitHub | [`titanium-claim`](https://github.com/chisss/titanium-claim) |

### 能力与边界

| 本服务负责 | 本服务不负责 |
|---|---|
| 理赔案件与状态机<br/>事故、损失和材料证据<br/>责任认定与赔付决定<br/>结案及追偿事实 | 保单合同解释原始定义<br/>资金渠道执行<br/>监管报表汇总 |
...
```mermaid
flowchart LR
    UP[Policy、Customer、Document] -->|API / Event| CURRENT[理赔域]
    CURRENT -->|API / Event| DOWN[Payment、Regulatory、Reinsurance]
```
...
| 模块 | 职责 |
|---|---|
| `titanium-claim-common` | 通用层：模块内枚举、异常和常量 |
| `titanium-claim-api` | API 层：服务间契约、Feign 接口和 DTO |
| `titanium-claim-domain` | 领域层：聚合、值对象、领域事件及 Port |
| `titanium-claim-application` | 应用层：用例编排、命令与查询协调 |
| `titanium-claim-infrastructure` | 基础设施层：Repository、远程 Adapter、消息与持久化 |
| `titanium-claim-query` | 查询层：CQRS 读模型与查询处理器 |
| `titanium-claim-web` | Web 层：REST 入口、请求校验和响应装配 |
| `titanium-claim-bootstrap` | 启动层：Spring Boot 入口、配置和 Liquibase |
...
```bash
git clone https://github.com/chisss/titanium-claim.git
cd titanium-claim
mvn clean verify
```
...
```bash
mvn -pl titanium-claim-bootstrap -am spring-boot:run
```

默认访问地址为 `http://localhost:8083/claim`。数据库、Redis、Kafka、下游服务地址及环境变量以 `titanium-claim-bootstrap/src/main/resources/application.yml` 为准。
