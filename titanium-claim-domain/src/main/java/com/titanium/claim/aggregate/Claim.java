...
public class Claim {
    ...
    public Claim(CreateClaimCommand command) {
        AggregateLifecycle.apply(new ClaimCreatedEvent(
                command.claimId(),
                command.customerId(),
                command.policyId(),
                command.claimNumber(),
    ...
    public void handle(UpdateClaimCommand command) {
        AggregateLifecycle.apply(new ClaimUpdatedEvent(
                command.claimId(),
                command.claimType(),
                command.incidentDate(),
                command.incidentDescription(),
    ...
    public void handle(ChangeClaimStatusCommand command) {
        ...
        AggregateLifecycle.apply(new ClaimStatusChangedEvent(
                command.claimId(),
                status,
                command.newStatus(),
                command.reason(),
    ...
    public void handle(SettleClaimCommand command) {
        if (status != ClaimStatus.APPROVED) {
            throw new ClaimStatusPreconditionException(
                    command.claimId(), status, "核赔结算", "APPROVED");
        }
        ClaimSettlement claimSettlement = ClaimSettlement.of(
                command.settledAmount(), command.payoutMethod(), command.payeeAccount(), command.conclusion());
        AggregateLifecycle.apply(new ClaimSettledEvent(
                command.claimId(),
                claimSettlement,
                LocalDateTime.now()
        ));
    ...
    /**
     * 提交查勘：推进理赔阶段至 SURVEY（车险/调查类案件）。
     * 须在 REPORT 阶段之后、APPROVAL 之前。
     */
    @CommandHandler
    public void handle(SubmitSurveyCommand command) {
        ensurePhaseBefore(ClaimPhase.SURVEY, ClaimPhase.APPROVAL);
        AggregateLifecycle.apply(new ClaimSurveySubmittedEvent(
                command.claimId(), command.survey(), ClaimPhase.SURVEY, LocalDateTime.now()));
    }

    /**
     * 提交定损：推进理赔阶段至 LOSS_ASSESS（车险按损案件）。
     * 须已完成查勘（SURVEY 阶段）。
     */
    @CommandHandler
    public void handle(SubmitLossAssessmentCommand command) {
        ...
        AggregateLifecycle.apply(new ClaimLossAssessedEvent(
                command.claimId(), command.lossAssessment(), ClaimPhase.LOSS_ASSESS, LocalDateTime.now()));
    }
    ...
    protected void on(ClaimLossAssessedEvent event) {
        ...
        this.phase = event.newPhase();
        this.updatedAt = event.assessedAt();
    }
    ...
    protected void on(ClaimSettledEvent event) {
        ...
        this.updatedAt = event.settledAt();
    }
}
